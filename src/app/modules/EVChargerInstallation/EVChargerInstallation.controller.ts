...
export const EVChargerInstallationController = {
  create: asyncHandler(async (req: Request, res: Response) => {
    ...
    const data = await EVChargerInstallationService.createEVChargerInstallationIntoDB(
  ...
  getMyAll: asyncHandler(async (req: Request, res: Response) => {
    const data = await EVChargerInstallationService.getMyAllEVChargerInstallationsFromDB(
      req.user._id.toString(),
    );

    sendResponse(res, {
      statusCode: httpStatus.OK,
      message: 'Your EV charger installation requests retrieved successfully!',
      data,
    });
  }),

  getSingle: asyncHandler(async (req: Request, res: Response) => {
    const data = await EVChargerInstallationService.getSingleEVChargerInstallationFromDB(
      req.user._id.toString(),
  ...
  updateSingle: asyncHandler(async (req: Request, res: Response) => {
    const data = await EVChargerInstallationService.updateSingleEVChargerInstallationIntoDB(
      req.user._id.toString(),
      req.params.id as string,
